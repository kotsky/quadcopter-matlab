...
switch FLY_MODE
    case 1
        Quad.Z_init = 30; % -Quad.Z_init + Quad.Correction_axe_Z = on plot
    case 2
        Quad.Z_init = 30; % -Quad.Z_init + Quad.Correction_axe_Z = on plot
    case 3
        Quad.Z_init = 50; % -Quad.Z_init + Quad.Correction_axe_Z = on plot
end
...
Quad.Z = Quad.Z_init;        % Initial position in Z direction GF (m)
Quad.X_BF = 0;     % Initial position in X direction BF (m)
Quad.Y_BF = 0;     % Initial position in Y direction BF (m)
Quad.Z_BF = Quad.Z_init;     % Initial position in the Z direction BF (m)
...
Quad.Z_des_GF = Quad.Z_init;         % desired value of Z in Global frame
Quad.X_des = 0;            % desired value of X in Body frame
Quad.Y_des = 0;            % desired value of Y in Body frame
Quad.Z_des = Quad.Z_init;            % desired value of Z in Body frame
...
% PID parameters
Quad.X_KP = .8;          % KP value in X position control 0.8
Quad.X_KI = .3;            % KI value in X position control
Quad.X_KD = -2.3;         % KD value in X position control -2.3
Quad.X_KI_lim = .1;         % Error to start calculating integral term

Quad.Y_KP = 0.75;          % KP value in Y position control 0.75
Quad.Y_KI = .3;            % KI value in Y position control
Quad.Y_KD = -1.85;         % KD value in Y position control -1.85
Quad.Y_KI_lim = .1;         % Error to start calculating integral term
...
%Quad.Z_KP = 1.5;    % KP value in altitude control 6 // 1.5 landing
Quad.Z_KI = 0.5;    % KI value in altitude control 0.5
Quad.Z_KD = -5;  % KD value in altitude control -5
...
Quad.p_KI = 0.5;     % KI value in pitch control
Quad.p_KD = -0;   % KD value in pitch control -.5
...
Quad.q_KI = 0.3;     % KI value in pitch control
Quad.q_KD = -0;   % KD value in pitch control -.5
...
Quad.r_KI = 0.5;     % KI value in pitch control
Quad.r_KD = -0;   % KD value in pitch control
